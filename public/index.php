...
$router->get(
    '/',
    function(){
        $_ENV['BODY'] =  "<h2>Welcome in my website</h2>";
    },
    'index'
);
...
$router->get('about',
    function(){
        $_ENV['BODY'] =  "<h2>A little bit about me</h2>";
    },
    'about'
);
$router->get('mycv',
    function(){
        $_ENV['BODY'] =  "<h2>My CV</h2>";
    },
    'mycv'
);
$router->get('contact',
    function(){
        $_ENV['BODY'] =  "<h2>Send me a message</h2>".
            '<form method="post">'.
            ' <label>Name</label>'.
...
$router->Post('contact',
    function(){
        $_ENV['BODY'] =  "<h2>Message sent</h2><pre>".print_r($_POST,true)."</pre>";
    },
    'contact'
);
...
$router->post(
    '/myProjects/:id',
    function($id){
        $_ENV['BODY'] =  "<h2>Post: Project $id</h2><pre>".print_r($_POST,true)."</pre>"; },
    'projectById'
)->with('id','\d+');
$router->get(
    '/myProjects/:id',
    function($id) use ($router, $include_body) { ;
        // $_ENV['BODY'] ="<p>{$router->url('projectById_get',['id'=>$id])}</p>".
        $_ENV['BODY'] ="<p>projectById_get, $id</p>".
            '<form method="post">'.
...
$router->get(
    '/myProjects/',
    function(){ $_ENV['BODY'] = "<h2>See all projects</h2>"; },
    'projects'
);
try {
    $router->run();
} catch (\HBoudaoud\Router\RouterException $e) {
    $_ENV['BODY']= "<div class='error'>{$e->getMessage()}</div>";
}

$include_body .= $_ENV['BODY'].
